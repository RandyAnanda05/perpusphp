...
if(isset($_GET['katalog'])) {
    ...
    $books = querykatalog($katalog);
    $count = queryperpus("SELECT COUNT(*) AS banyak_buku FROM buku WHERE LEFT(id, 2) = LEFT('$katalog',2) OR LEFT(id, 3) = LEFT('$katalog',3) ");
} else {
    echo "<script>
    alert('erorr saat query, hubungi petugas');
      ...
      <div class="bg-gray-700">
        <div class="flex flex-row w-9/12 py-5 mx-auto">
          <div class="text-white font-bold text-md flex flex-start basis-1/2">
            ...
            <div>Daftar Koleksi <?=$_GET["katalog"]?> Sebanyak <span><?=$count[0]['banyak_buku']?> </span></div>
          </div>
